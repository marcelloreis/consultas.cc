...
<?php if(count($entity['Family']['mother'])):?>
    <?php foreach($entity['Family']['mother'] as $k => $v):?>
        <blockquote>
            <h4>
                <?php echo $this->Html->link('Mãe', '#', array('class' => 'btn btn-success'))?>
                <?php echo $this->Html->link($v['Entity']['name'], array('controller' => 'entities', 'action' => 'index', 'plugin' => false, $v['Entity']['id'], '#' => 'entity-main'))?>
            </h4>
