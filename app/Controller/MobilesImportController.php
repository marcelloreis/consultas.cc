 ...
 */
class MobilesImportController extends AppController {
	public $uses = array(
		"Import", 
		"NattMobile", 
		"Imobile",
	...
	public function run($uf=null){
		...
		*/
		if($uf){
			...
			/**
			* Carrega os models com o nome da tabela
			*/
			...
			*/
			...
			$this->db['landline'] = $this->Imobile->getDataSource();
			$this->db['address'] = $this->Imobile->getDataSource();
			$this->db['zipcode'] = $this->Imobile->getDataSource();
					...
					if(isset($entity['telefone'])){
						foreach ($entity['telefone'] as $k => $v) {
							...
							*/
							...
							/**
							* Desmembra o DDD do Celular
							*/
							$this->AppImport->timing_ini(TUNING_LANDLINE_LOAD);
							$ddd_telefone = $v['TELEFONE'];
							$ddd = $this->AppImport->getDDDMobile($v['TELEFONE']);
							$telefone = $this->AppImport->getMobile($v['TELEFONE']);
							...
							*/
							...
							$this->importMobile($data, $v['TELEFONE']);
							$this->AppImport->timing_end();
							...
							*/
							...
							/**
							* Amarra os registros Entidade, Celular, CEP e Endereço na tabela associations
							*/
	...
	* @return void
	*/
	private function importMobile($mobile){
		/**
	...
	private function importAssociation($association){
		...
		*/
		...
		if(
			...
			&& 
			(
				(!empty($association['Iassociation']['mobile_id']) && $association['Iassociation']['mobile_id'] != '0') 
				|| 
