...
class NattFixoPessoa extends AppModelClean {
    ...
    public function next($source, $row_count){
        $data = array();

        switch ($source) {
            case 'binary':
                $data = $this->next_binary($row_count);
                break;
            case 'text':
                /**
                * Carrega o diretorio para os arquivos com os dados em txt
                */
                $this->folder_source = "/var/www/consultas.cc/_db/source/";
    ...
    private function next_binary($row_count){
        $map = array();
    ...
    private function txt2array($path){
        ...
        */
        ...
        while (!feof ($txt)) {
            ...
            */
            @$cpf_cnpj = !empty($v[$pos_lay['DOC']])?trim($v[$pos_lay['DOC']]):null;
            ...
            @$telefone = !empty($v[$pos_lay['TEL']])?$v[$pos_lay['TEL']]:null;
            @$tel_full = "0{$ddd}{$telefone}";
            @$cep = !empty($v[$pos_lay['CEP']])?trim($v[$pos_lay['CEP']]):null;
            @$cod_end = null;
            @$complemento = !empty($v[$pos_lay['COMPL']])?trim($v[$pos_lay['COMPL']]):null;
            @$numero = !empty($v[$pos_lay['NUM']])?trim($v[$pos_lay['NUM']]):null;
            @$data_atualizacao = !empty($v[$pos_lay['DATA_ATUALIZACAO']])?'2012-01-01':null;
            @$rua = !empty($v[$pos_lay['TP_LOG']])?trim($v[$pos_lay['TP_LOG']]):null;
            @$nome_rua = !empty($v[$pos_lay['LOGR']])?trim($v[$pos_lay['LOGR']]):null;
            @$bairro = !empty($v[$pos_lay['BAIRRO']])?trim($v[$pos_lay['BAIRRO']]):null;
    ...
    private function next_text($folder=false) {
        /**
        ...
        */
        $folder = $folder?$folder:$this->folder_source;
