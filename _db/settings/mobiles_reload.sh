...
mysql -u $user -p$password -e 'INSERT INTO consultas_cc.mobiles SELECT * FROM consultas_cc.i_mobiles;'
...
mysql -u $user -p$password -e 'TRUNCATE TABLE consultas_cc.i_mobiles;'
...
ai=`mysql -u $user -p$password -e "SELECT AUTO_INCREMENT FROM  INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'consultas_cc' AND TABLE_NAME = 'mobiles';" | sed 's/[^0-9]*//g'`
mysql -u $user -p$password -e "ALTER TABLE consultas_cc.i_mobiles AUTO_INCREMENT = $ai;"
